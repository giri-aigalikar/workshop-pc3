...
extern bool brakeRequest;
extern float currentDistance;// = 0.0; // m (ID 259 //DISTANCE_TO_OBJECT 0x103
extern float relativeVelocity;// = 0.0; // m/s (ID 260) //VEHICLE_VELOCITY 0x104
extern float ttc;// = 0.0;
extern bool isStoppedAfterBraking;
...
User_DeclQuants(void)
{
	...
	DDefFloat(NULL, "TTC", "s",&ttc, DVA_None);
	DDefFloat(NULL, "currentDistance", "m",&currentDistance, DVA_None);
	DDefFloat(NULL, "relativeVelocity", "m/s",&relativeVelocity, DVA_None);
	DDefInt(NULL, "brakeRequest", "",&brakeRequest, DVA_None);
}
...
User_VehicleControl_Calc(double dt)
{
	...
	vehicleSpeed = Car.ConBdy1.v;
    currentDistance = pOS->relvTarget.RefPnt.ds_p;
    relativeVelocity = fabs(pOS->relvTarget.RefPnt.dv_p);

	task_10ms();
 
	if (brakeRequest || isStoppedAfterBraking){
