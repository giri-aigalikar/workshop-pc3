function [] = update_CarAndTrailer_Forces_CleanUp(src_mdl, trg_mdl)
...
Parent_src = 'generic_src/CarMaker/IPG Vehicle';
Parent_trg = append(trg_mdl,'/CarMaker/IPG Vehicle');
...
for (j=1:numel(System_matches.trgBlck))
    if (strcmp(get_param(System_matches.trgBlck{j},'Name'),'Vehicle') && strcmp(get_param(System_matches.trgBlck{j}, 'Parent'), Parent_trg_temp))
        Parent_trg = append(trg_mdl,'/CarMaker/Vehicle/IPG Vehicle');
    end
end
...
Parent_src = 'generic_src/CarMaker/IPG Vehicle/CarAndTrailer';
