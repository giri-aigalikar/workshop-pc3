...
for (i=1:numel(subs))
    ...
    % substitute block
    if (isfield(opts,'PosFromSrc') && opts.PosFromSrc == 1)
        pos = get_param(srcBlck, 'Position');
    else
        pos = get_param(trgBlck, 'Position');
    end

    name      = get_param(srcBlck, 'Name');
    ...
    % rename block
    if (isfield(opts,'NameFromSrc') && opts.NameFromSrc == 1)
        set_param(trgBlck, 'Name', name);
    end

    % Rearrange system blocks
    %if (i == numel(subs))
	%Simulink.BlockDiagram.arrangeSystem(trgSys) % Might cause error when called on the same Simulink connection twice
    %end
end
end
