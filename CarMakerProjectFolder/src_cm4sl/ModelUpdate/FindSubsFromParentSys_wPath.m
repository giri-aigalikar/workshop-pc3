function [subs] = FindSubsFromParentSys_wPath (blck, sys, ModSys, ModifiedPort, PortKind, Parent_src, Parent_trg)
...
for (j=1:numel(srcBlck))
    if (strcmp(get_param(srcSys{j},'Name'),sys) && strcmp(get_param(srcSys{j}, 'Parent'), Parent_src))
        srcSys_static = srcSys{j};
    end
end

if (ModifiedPort == -1)
    % replace the block itself
    for (j=1:numel(trgBlck))
        % Only update instance in current sys
        if (strcmp(get_param(trgSys{j},'Name'),sys) && strcmp(get_param(trgSys{j}, 'Parent'), Parent_trg))
